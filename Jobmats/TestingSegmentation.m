...
SkullstrippedList = cell(1,length(biasregTestset)*length(biasfwhmTestset)*length(ngausTestset));
for i = 1:length(biasregTestset)
    for ii = 1:length(biasfwhmTestset)
        ...
        for iii = 1:length(ngausTestset)
            ...
            SkullstrippedList{d}=[newdirName filesep 'skullstripped.img'];
            d=d+1;
