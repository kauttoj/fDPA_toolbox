...
[filename, pathname] = uigetfile({'*.img;*.mat', 'All image files (*.img; *.mat)'; ...
												'*.mat','MAT images (*.mat)'; ...
												'*.img', 'ANALYZE or NIFTI images(*.img)'}, ...
												'Pick a user''s  image');
if ~(filename==0)
...
if (~isequal(ADir , 0)) &&( (size(handles.Cfg.DataDirs, 1)==0)||(0==length(strmatch(ADir,handles.Cfg.DataDirs( : , 1),'exact' ) ) ))
	 ...
	 theVolumnCount =CheckDataDir(handles.Cfg.DataDirs{tmpSize(1),1} );	
	 if (theVolumnCount<=0),
...
Result ={};
for x=size(handles.Cfg.DataImgs):-1:1
...
olddir=pwd;
[GroupSeries, VoxelSize, ImgFileList, Header] =rest_to4d(GroupDir);
cd(olddir);
rest_waitbar;
