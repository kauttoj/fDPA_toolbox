...
%   Last Modified by Yan Chao-Gan 080808: also support NIFTI images.
gui_Singleton = 1;
...
% hObject    handle to pmnuMisc (see GCBO)
	...
	case {1, 2, 4, 8},	%Misc or '---'
		%Do nothing
	case 3, %'Retrieve averaged time course from selected ROI definition'
		if prod(size(handles.ROISetList))>0 && size(handles.ROISetList, 1)>0,
			theIndex =get(handles.listROI, 'Value');
			AROIDef  =handles.ROISetList{theIndex};
			RetrieveTimeCourseFromROIDefinition(AROIDef);
		end
	case 5, %'Export List of ROI definitions'
		if  prod(size(handles.ROISetList))>0 && size(handles.ROISetList, 1)>0,
	...
	case 6, %'Import List of ROI definitions'
	...
	try	
		...
		if rest_SphereROI( 'IsBallDefinition', AROIDef),
			...
			if any(filename~=0) && ischar(filename) && length(filename)>4 ,	% not canceled and legal				
				%Get the 4D brain
				[the4DBrain, VoxelSize, ImgFileList, Header] =rest_to4d(pathname);				
				%Generate the mask				
