function [ResultMap1,ResultMap2] = rest_gca_residual(DataDir,AMaskFilename, AROIDef,AResultFilename, ACovariablesDef,Order)
...
%	
	...
	if iscell(AROIDef),	%ROI wise, compute corelations between regions	
        ...
        if size(theROITimeCourses,2)~=2,
           ...
           [ResultMap1,ResultMap2] = restgca_FROI(theROITimeCourses,Order,theCovariables);
        end
	...
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%	 
		...
		%for x=1:(floor(nDim1/NumPieces_Dim1) +1)
			...
			for y=1:size(AROITimeCourse, 2),
				...
				[ResultMap1,ResultMap2] = restgca_residual(AROITimeCourse(:, y), theDim1Volume4D,Order,theCovariables);
		...
		clear theDim1Volume4D theDim1Mask4D ResultMap1 ResultMap2;
		...
		for  y=1:size(AROITimeCourse, 2),
			...
			elseif size(AROITimeCourse, 2)==1,
				...
				fprintf('\n\t Saving GCA map.\tWait...');	
                AResultFilename1=[AResultFilename,'x2y'];
                AResultFilename2=[AResultFilename,'y2x'];
...
function [ResultMap1,ResultMap2] = restgca_residual(AROITimeCourse, ABrain4D,Order,theCovariables)
...
function [ResultMap1,ResultMap2] = restgca_FROI(theROITimeCourses,Order,theCovariables)
