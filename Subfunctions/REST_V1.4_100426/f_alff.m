...
%   Revised by YAN Chao-Gan, 090321. (1) Added the fALFF module, thank Dr. CHENG Wen-Lian for the helpful work. (2) Result data will be saved in the format 'single'.
%   Last Revised by YAN Chao-Gan, 100420. Fixed a bug in calculating the frequency band.


	if nargin~=6
        error(' Error using ==> f_alff. 6 arguments wanted.'); 
    end
	...
	theTempDatasetDirName =sprintf('fALFF_%d_%s', fix((1e4) *rem(now, 1) ),rest_misc('GetCurrentUser'));	
...
%	for x=1:(floor(nDim1/NumPieces_Dim1) +1)
		...
		% for y=1:sampleLength % Does fALFF need this routine?
		...
		theDim1Volume4D_without_filter=theDim1Volume4D;% Save the FFT (without filter)  Revised by YAN Chao-Gan, 20090321. Thank Dr. CHENG Wen-Lian for the helpful work.
		...
		%Get the Square root of the power spectrum between 0.01 and 0.08, i.e., fALFF
		theDim1Volume4D =sqrt(theDim1Volume4D);
		theDim1Volume4D =sum(theDim1Volume4D,4);
		
		%Compute the fALFF. Revised by YAN Chao-Gan, 20090321. Thank Dr. CHENG Wen-Lian for the helpful work.
		%The square root was calculated at each frequency of the power spectrum. The sum of amplitude across 0.01¨C0.08 Hz was divided by that across the entire frequency range, i.e., 0¨C0.25 Hz.	
		%Get the amplitude only in one of the symmetric sides after FFT
		theDim1Volume4D_without_filter =theDim1Volume4D_without_filter(:, :, :, 2:(paddedLength/2+1));
		theDim1Volume4D_without_filter =abs(theDim1Volume4D_without_filter);
		%Get the Power Spectrum, double it because I only want one side of both sides
		theDim1Volume4D_without_filter =2*(theDim1Volume4D_without_filter .* theDim1Volume4D_without_filter) /sampleLength;
		theDim1Volume4D_without_filter(:,:,:, end) =theDim1Volume4D_without_filter(:,:,:, end) /2;
		%The DC component didn't double because it didn't have its symetric side
		%theDim1Volume4D_without_filter(:,:,:,1) =theDim1Volume4D_without_filter(:,:,:,1)/2;
		%Get the Square root of the power spectrum between 0.01 and 0.08, i.e., fALFF
		theDim1Volume4D_without_filter =sqrt(theDim1Volume4D_without_filter);
		theDim1Volume4D_without_filter =sum(theDim1Volume4D_without_filter,4);
		
		temp_pos=find(theDim1Volume4D_without_filter);
		theDim1Volume4D_fALFF=zeros(size(theDim1Volume4D_without_filter));
		theDim1Volume4D_fALFF(temp_pos) =theDim1Volume4D(temp_pos) ./ theDim1Volume4D_without_filter(temp_pos);
		theDim1Volume4D=theDim1Volume4D_fALFF;				
	...
	clear theDim1Volume4D theTrend_Intercept theTrend_Slope theDim1FilterMask4D oneAxialSlice theDim1Volume4D_fALFF theDim1Volume4D_without_filter;
	...
	for x=1:(NumComputingCount)
		rest_waitbar(x/(floor(nDim1/NumPieces_Dim1)+1), ...
					'fALFF 3D Brain reconstructing. Please wait...', ...
					'REST working','Child','NeedCancelBtn');
...
function Save1stDimPieces(ATempDir, A4DVolume, AFilenamePrefix)
	...
	%for x = 1:(floor(size(A4DVolume,1)/NumPieces_Dim1)+1)
		rest_waitbar(x/(floor(size(A4DVolume,1)/NumPieces_Dim1)+1), ...
					'Cut one Big 3D+time Dataset into pieces of 3D+time Dataset Before fALFF. Please wait...', ...
					'REST working','Child','NeedCancelBtn');
