	...
	try
		...
		elseif get(REST_SetROI_Cfg.hSlectNonSphere, 'Value'),			
			...
			if get(REST_SetROI_Cfg.hFromTMap, 'Value'),
				[filename, pathname] = uigetfile({'*.img', 'ANALYZE or NIFTI files (*.img)'},'Pick the  statistical map');%100329	
			...
			elseif get(REST_SetROI_Cfg.hFromUserDefinedMask, 'Value'),
				[filename, pathname] = uigetfile({'*.img', 'ANALYZE or NIFTI files (*.img)'},'Pick the user defined mask file(ANALYZE or NIFTI FORMAT)');
