...
msgbox({'Correlation Analysis:';...
    'If only the group images and the seed series are specified, then perform Pearson''s correlation analysis.';...
    ...
    'Other covariate can be also specified as text files. (E.g. age, brain size, IQ etc.)';...
...
if lenC(1) >=1
    msgbox('Only one other covariates file is allowed in Correlation Analysis.','Warning');
else
    [filename, pathname] = uigetfile({'*.txt', 'Other covariates (*.txt)';}, ...
        'Pick a user''s  txt');
    if ~(filename==0)
...
if lenC(1) >=1
    msgbox('Only one seed series file is allowed in Correlation Analysis.','Warning');
else
    [filename, pathname] = uigetfile({'*.txt', 'seed series (*.txt)';}, ...
        'Pick a txt file of seed series');
    if ~(filename==0)
...
uimenu(handles.hContextMenui, 'Label', 'Add a seed series', 'Callback', get(handles.btnAddSeedSeries, 'Callback'));
uimenu(handles.hContextMenui, 'Label', 'Remove selected seed series', 'Callback', 'rest_corr_gui(''DeleteSelectedSeedSeries'',gcbo,[], guidata(gcbo))');
uimenu(handles.hContextMenui, 'Label', '=============================');
uimenu(handles.hContextMenui, 'Label', 'Remove all seed series', 'Callback', 'rest_corr_gui(''ClearSeedSeries'',gcbo,[], guidata(gcbo))');
...
if (~isequal(ADir , 0)) &&( (size(handles.Cfg.GroupDirs, 1)==0)||(0==length(strmatch(ADir,handles.Cfg.GroupDirs( : , 1),'exact' ) ) ))
    ...
    theVolumnCount =CheckDataDir(handles.Cfg.GroupDirs{tmpSize(1),1} );
    if (theVolumnCount<=0),
...
if (~isequal(ADir , 0)) &&( (size(handles.Cfg.CovariateDirs, 1)==0)||(0==length(strmatch(ADir,handles.Cfg.CovariateDirs( : , 1),'exact' ) ) ))
    ...
    theVolumnCount =CheckDataDir(handles.Cfg.CovariateDirs{tmpSize(1),1} );
    if (theVolumnCount<=0),
        if isappdata(0, 'FC_DoingRecursiveDir') && getappdata(0, 'FC_DoingRecursiveDir'),
        else
            fprintf('There is no data or non-data files in this directory:\n%s\nPlease re-select\n\n', ADir);
...
for x=size(handles.Cfg.OtherTxt, 1):-1:1
    Result =[{sprintf('OC%d: 1 txt %s',x,handles.Cfg.OtherTxt{x})} ;Result];
end

function Result=GetInputSeedSeriesDisplayList(handles)
Result ={};
for x=size(handles.Cfg.SeedSeriesTxt, 1):-1:1
    Result =[{sprintf('Seed Series %d: 1 txt %s',x,handles.Cfg.SeedSeriesTxt{x})} ;Result];
end
