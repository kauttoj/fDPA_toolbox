...
%   Last Modified by DONG Zhang-Ye and YAN Chao-Gan 100420: Fixed a bug in displaying overlay with different bounding box from those of underlay.
%------------------------------------------------------------------------------------------------------------------------------
...
%Input: Need AConfig.Filename specified	
	try
        ...
        [pathstr, name, ext, versn] = fileparts(AConfig.Filename);  %YAN Chao-Gan 100420 added. Change the first voxel of ch2bet.nii since it can not be displayed in a right way.
...
function Result =ScaleOverlay2TrueColor(AConfig, AOverlayImg, AColorMap)
	[pathstr, name, ext, versn] = fileparts(AConfig.Overlay.Filename);
...
function Result =LoadOverlay(AConfig, AFilename)
	...
	[pathstr, name, ext, versn] = fileparts(Result.Overlay.Filename);
	if strcmpi(name, 'aal'),		
