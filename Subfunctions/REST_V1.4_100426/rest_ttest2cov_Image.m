...
for i=1:GroupNumber
    [AllVolume,VoxelSize,theImgFileList, Header] =rest_to4d(DependentDirs{i});
    ...
    if ~isempty(CovariateDirs)
        [AllVolume,VoxelSize,theImgFileList, Header_Covariate] =rest_to4d(CovariateDirs{i});
    ...
    GroupLabel=[GroupLabel;ones(size(theImgFileList,1),1)*i];
    clear AllVolume
end
...
for i=1:nDim1
    rest_waitbar(i/nDim1, 'Two Sapmle T Test Calculating...', 'Two Sapmle T Test','Child');
    fprintf('.');
    for j=1:nDim2
        for k=1:nDim3
            if MaskData(i,j,k)
...
rest_waitbar;
fprintf('\n\tTwo Sapmle T Test Calculation finished.\n');
