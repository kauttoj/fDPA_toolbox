...
if iscell(AROIDef),	%ROI wise, compute corelations between regions
    ...
    save([OutDir,filesep,SubID,'_ROISeries.mat'],'theROITimeCourses'); %% Revised by YAN Chao-Gan, 100420. Change the output name from "ROITimeCourses" to "ROISeries'. 
    save([OutDir,filesep,SubID,'_ROISeries.txt'],'theROITimeCourses', '-ASCII', '-DOUBLE','-TABS'); %% Revised by YAN Chao-Gan, 100420. Change the output name from "ROITimeCourses" to "ROISeries'. 
